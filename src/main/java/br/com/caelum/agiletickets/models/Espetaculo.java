	...
	public List<Sessao> criaSessoes(LocalDate inicio, LocalDate fim, LocalTime horario, Periodicidade periodicidade) {
		...
		if (!fim.isBefore(inicio)) {
			if (periodicidade.equals(Periodicidade.DIARIA) ) {
				long numeroDeSessoes = Days.daysBetween(inicio, fim).getDays() ;
				
				for( long contador = 0;contador<=numeroDeSessoes; contador++) {
					Sessao sessao = new Sessao();
					sessoes.add(sessao);				
				}
			} else if (periodicidade.equals(Periodicidade.SEMANAL) ) {
				long numeroDeSessoes = Weeks.weeksBetween(inicio, fim).getWeeks() ;
